...
describe("Cleanup copy", () => {
  ...
  it("keeps the cleanup screen focused on the 30-day restore bin flow", () => {
    ...
    expect(source).toContain("restoreBinExpiryInsight(restoreItems)");
    ...
    expect(source).toContain("expiryInsight.message");
    expect(source).toContain("expiryInsight.detail");
    expect(source).not.toContain("다음 항목은");
