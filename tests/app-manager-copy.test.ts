...
describe("AppManager uninstall copy", () => {
  ...
  it("includes registry backup undo in the recent cleanup flow", () => {
    ...
    expect(source).toContain("앱 삭제 흔적/기본 앱 목록/PATH 경로/실행 경로/앱 연결/우클릭 메뉴/서비스/시작 항목 백업");
    expect(source).toContain("30일 동안 되돌릴 수 있게 백업해요");
    expect(source).toContain("폴더·바로가기와 시작 항목, 서비스, 앱 삭제 흔적과 기본 앱 목록, PATH 경로, 앱 연결, 우클릭 메뉴 흔적은 30일 안에 되돌릴 수 있게");
