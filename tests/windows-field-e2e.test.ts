...
const FIELD_REQUIREMENTS = {
  ...
  comInprocServerLeftover: fieldRequirements[15],
  contextMenuKey: fieldRequirements[16],
  shellExtensionKey: fieldRequirements[17],
  explorerExtensionKey: fieldRequirements[18],
  userPathSegment: fieldRequirements[19],
...
const FIELD_COM_INPROC_CLSID = `{A6E0BCA2-2CC0-4B8C-A29F-${FIELD_GUID_SUFFIX}}`;
...
const COM_INPROC_SERVER_KEY = `HKCU\\Software\\Classes\\CLSID\\${FIELD_COM_INPROC_CLSID}`;
...
const EXPLORER_EXTENSION_KEY =
  `HKCU\\Software\\Classes\\Directory\\shellex\\CopyHookHandlers\\${FIELD_VALUE_NAME}`;
...
const APP_EXECUTION_HISTORY_KEY =
...
async function registryValues(
  ...
  try {
    const { stdout } = await runReg(["query", keyPath]);
    const values: Array<{ valueName: string; type: string; data: string }> = [];
    for (const line of stdout.split(/\r?\n/)) {
      const row = line.trim();
      if (!row || /^HKEY_/i.test(row)) continue;
      const parts = row.split(/\s{2,}/);
      const typeIndex = parts.findIndex((part) => /^REG_/i.test(part));
      if (typeIndex <= 0) continue;
      const valueName = parts.slice(0, typeIndex).join("  ");
      const type = parts[typeIndex];
      const data = parts.slice(typeIndex + 1).join("  ");
      if (valueName && type) values.push({ valueName, type, data });
    }
    return values;
  } catch {
    return [];
...
fieldDescribe("Windows field E2E: restore bin and startup controls", () => {
  ...
  afterEach(async () => {
    ...
    await runReg(["delete", COM_INPROC_SERVER_KEY, "/f"]).catch(() => {});
  ...
  it("backs up, removes, and restores isolated default-app, app path, app connection, file association, protocol handler, browser helper, context menu, and right-click extension registry traces", async () => {
    ...
    await runReg([
      "add",
      `${COM_INPROC_SERVER_KEY}\\InprocServer32`,
      "/ve",
      "/t",
      "REG_SZ",
      "/d",
      `C:\\Program Files\\${FIELD_VALUE_NAME}\\${FIELD_VALUE_NAME}Shell.dll`,
      "/f"
    ]);
    await runReg([
      "add",
    ...
    await runReg([
      "add",
      EXPLORER_EXTENSION_KEY,
      "/ve",
      "/t",
      "REG_SZ",
      "/d",
      FIELD_COM_INPROC_CLSID,
      "/f"
    ]);
    await setRegistryValue(
      APP_EXECUTION_HISTORY_KEY,
      FIELD_EXECUTION_HISTORY_VALUE,
      "REG_BINARY",
      "01000000"
    );
    ...
    expect(await registryKeyExists(COM_INPROC_SERVER_KEY)).toBe(true);
      ...
      registryRunner: {
        listSubKeys: async (keyPath: string) => {
          if (
            ...
            return [FILE_ASSOCIATION_OPEN_COMMAND_KEY.split("\\").pop()!];
          }
          if (keyPath === "HKCU\\Software\\Classes\\CLSID") {
            const keys: string[] = [];
            if (await registryKeyExists(COM_LOCAL_SERVER_KEY)) keys.push(FIELD_COM_CLSID);
            if (await registryKeyExists(COM_INPROC_SERVER_KEY)) keys.push(FIELD_COM_INPROC_CLSID);
            return keys;
          }
          if (
            keyPath === "HKCU\\Software\\Classes\\Directory\\shellex\\CopyHookHandlers" &&
            (await registryKeyExists(EXPLORER_EXTENSION_KEY))
          ) {
    ...
    const comInprocServerLeftover = openCommandLeftovers.groups
      .flatMap((group) => group.paths)
      .find((path) => path.kind === "com-inproc-server-registry" && path.path === COM_INPROC_SERVER_KEY);
    expect(comInprocServerLeftover).toMatchObject({
      kind: "com-inproc-server-registry",
      path: COM_INPROC_SERVER_KEY,
      exists: true
    });
    const explorerExtensionLeftover = openCommandLeftovers.groups
      .flatMap((group) => group.paths)
      .find((path) => path.kind === "explorer-extension-registry" && path.path === EXPLORER_EXTENSION_KEY);
    expect(explorerExtensionLeftover).toMatchObject({
      kind: "explorer-extension-registry",
      path: EXPLORER_EXTENSION_KEY,
      exists: true
    });
    const executionHistoryLeftover = openCommandLeftovers.groups
      .flatMap((group) => group.paths)
      .find((path) => path.kind === "app-execution-history-registry" && path.path === APP_EXECUTION_HISTORY_KEY);
    expect(executionHistoryLeftover).toMatchObject({
    ...
    const comInprocServerBackup = await backupAndDeleteRegistryKey({
      userDataDir,
      keyPath: COM_INPROC_SERVER_KEY,
    ...
    const explorerExtensionBackup = await backupAndDeleteRegistryKey({
      userDataDir,
      keyPath: EXPLORER_EXTENSION_KEY,
      backupKind: "explorer-extension-key",
      now: () => firstAt,
      app: { name: FIELD_VALUE_NAME, publisher: "FormatBuddy Field E2E" }
    });
    const executionHistoryBackup = await backupAndDeleteRegistryValue({
      userDataDir,
      keyPath: APP_EXECUTION_HISTORY_KEY,
      valueName: FIELD_EXECUTION_HISTORY_VALUE,
    ...
    expect(comInprocServerBackup.backupKind).toBe("com-inproc-server-key");
    ...
    expect(await registryKeyExists(COM_INPROC_SERVER_KEY)).toBe(false);
    ...
    const restoredComInprocServer = await restoreRegistryBackup({
      userDataDir,
      backupId: comInprocServerBackup.id,
      now: () => restoredAt
    });
    ...
    const restoredExplorerExtension = await restoreRegistryBackup({
      userDataDir,
      backupId: explorerExtensionBackup.id,
      now: () => restoredAt
    });
    const restoredExecutionHistory = await restoreRegistryBackup({
      userDataDir,
    ...
    expect(restoredComInprocServer.status).toBe("restored");
    ...
    expect(restoredComInprocServer.entry?.backupKind).toBe("com-inproc-server-key");
    ...
    proveFieldRequirement(FIELD_REQUIREMENTS.comInprocServerLeftover);
