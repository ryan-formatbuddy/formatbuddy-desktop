...
describe("planAppLeftovers", () => {
  ...
  it("shows App Paths registry leftovers as selectable backup-first candidates after uninstall follow-up", async () => {
    const appPathRegistryKey =
      "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\AcmeNotes.exe";
    const registryRunner = {
      keyExists: vi.fn(async (keyPath: string) => keyPath === appPathRegistryKey)
    };
    ...
    expect(registryRunner.keyExists).toHaveBeenCalledWith(appPathRegistryKey);
    const appPathCandidate = snapshot.groups[0].paths.find((p) => p.path === appPathRegistryKey);
    expect(appPathCandidate).toMatchObject({
      kind: "app-path-registry",
      exists: true
    });
    expect(appPathCandidate?.protectedBy).toBeUndefined();
  });

  it("shows default-app list registry leftovers as selectable backup-first candidates after uninstall follow-up", async () => {
    const registeredApplicationsKey = "HKCU\\Software\\RegisteredApplications";
    const registeredApplicationsValue = "Acme Notes";
    const registryRunner = {
      valueExists: vi.fn(
        async (keyPath: string, valueName: string) =>
    ...
    expect(registryRunner.valueExists).toHaveBeenCalledWith(
      registeredApplicationsKey,
      registeredApplicationsValue
    );
    const registeredAppCandidate = snapshot.groups[0].paths.find(
      (p) => p.path === registeredApplicationsKey && p.registryValueName === registeredApplicationsValue
    );
    expect(registeredAppCandidate).toMatchObject({
  ...
  it("shows app environment variable leftovers as selectable backup-first candidates after uninstall follow-up", async () => {
    const environmentKey = "HKCU\\Environment";
    const environmentValue = "ACME_NOTES_HOME";
    const registryRunner = {
      queryValue: vi.fn(async (keyPath: string, valueName: string) =>
        keyPath === environmentKey && valueName === environmentValue
          ? { type: "REG_SZ", data: "C:\\Program Files\\Acme Notes" }
          : undefined
      )
    };
    ...
    expect(registryRunner.queryValue).toHaveBeenCalledWith(environmentKey, environmentValue);
    const environmentVariableCandidate = snapshot.groups[0].paths.find(
      (p) => p.path === environmentKey && p.registryValueName === environmentValue
    );
    expect(environmentVariableCandidate).toMatchObject({
  ...
  it("shows app display cache leftovers as selectable backup-first candidates after uninstall follow-up", async () => {
    const displayCacheKey =
      "HKCU\\Software\\Classes\\Local Settings\\Software\\Microsoft\\Windows\\Shell\\MuiCache";
    const displayCacheValue = "C:\\Program Files\\Acme Notes\\AcmeNotes.exe.FriendlyAppName";
    const registryRunner = {
      exportKey: vi.fn(async () => undefined),
      deleteKey: vi.fn(async () => undefined),
      listValues: vi.fn(async (keyPath: string) =>
        keyPath === displayCacheKey
          ? [
              {
                valueName: displayCacheValue,
                type: "REG_SZ",
                data: "Acme Notes"
              },
              {
    ...
    expect(registryRunner.listValues).toHaveBeenCalledWith(displayCacheKey);
    const displayCacheCandidate = snapshot.groups[0].paths.find(
      (p) => p.path === displayCacheKey && p.registryValueName === displayCacheValue
    );
    expect(displayCacheCandidate).toMatchObject({
      kind: "app-display-cache-registry",
      registryValueName: displayCacheValue,
      exists: true
    });
    expect(displayCacheCandidate?.protectedBy).toBeUndefined();
    expect(
      snapshot.groups[0].paths.some((p) => p.registryValueName === "C:\\Windows\\System32\\notepad.exe.FriendlyAppName")
    ).toBe(false);
  });
  ...
  it("shows app connection registry leftovers as selectable backup-first candidates after uninstall follow-up", async () => {
    ...
    const registryRunner = {
      keyExists: vi.fn(async (keyPath: string) => keyPath === openWithRegistryKey)
    };
    ...
    expect(registryRunner.keyExists).toHaveBeenCalledWith(openWithRegistryKey);
    const openWithCandidate = snapshot.groups[0].paths.find((p) => p.path === openWithRegistryKey);
    expect(openWithCandidate).toMatchObject({
  ...
  it("shows protocol handler registry leftovers as selectable backup-first candidates after uninstall follow-up", async () => {
    ...
    const registryRunner = {
      valueExists: vi.fn(
        async (keyPath: string, valueName: string) =>
          keyPath === protocolRegistryKey && valueName === "URL Protocol"
      )
    };
    ...
    expect(registryRunner.valueExists).toHaveBeenCalledWith(protocolRegistryKey, "URL Protocol");
    const protocolCandidate = snapshot.groups[0].paths.find((p) => p.path === protocolRegistryKey);
    expect(protocolCandidate).toMatchObject({
      kind: "protocol-handler-registry",
      exists: true
    });
  ...
  it("shows right-click menu registry leftovers as selectable backup-first candidates after uninstall follow-up", async () => {
    ...
    const registryRunner = {
      keyExists: vi.fn(async (keyPath: string) => keyPath === contextMenuRegistryKey)
    };
    ...
    expect(registryRunner.keyExists).toHaveBeenCalledWith(contextMenuRegistryKey);
    const contextMenuCandidate = snapshot.groups[0].paths.find(
      (p) => p.path === contextMenuRegistryKey
    );
    expect(contextMenuCandidate).toMatchObject({
  ...
  it("backs up and deletes selected App Paths registry leftovers after uninstall follow-up", async () => {
    const appPathRegistryKey =
      "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\AcmeNotes.exe";
    let keyExists = true;
    const registryRunner = {
      keyExists: vi.fn(async (keyPath: string) => keyExists && keyPath === appPathRegistryKey),
      exportKey: vi.fn(async (_keyPath: string, backupPath: string) => {
        await fs.mkdir(dirname(backupPath), { recursive: true });
        await fs.writeFile(backupPath, registryBackupContentFor(_keyPath), "utf8");
      }),
    ...
    const path = snapshot.groups[0].paths.find((p) => p.path === appPathRegistryKey)!;
    expect(path).toMatchObject({ kind: "app-path-registry", exists: true });
    ...
    expect(registryRunner.deleteKey).toHaveBeenCalledWith(appPathRegistryKey);
    expect(result.removedItems).toHaveLength(1);
    expect(result.removedItems[0]).toMatchObject({
      itemId: path.id,
      path: appPathRegistryKey,
    ...
    expect(registryBackups.entries[0]).toMatchObject({
      appName: "Acme Notes",
      appPublisher: "Acme Corp.",
      keyPath: appPathRegistryKey,
      backupKind: "app-path-key"
    });
  });

  it("backs up and deletes selected default-app list registry leftovers after uninstall follow-up", async () => {
    const registeredApplicationsKey = "HKCU\\Software\\RegisteredApplications";
    const registeredApplicationsValue = "Acme Notes";
    ...
    expect(path).toMatchObject({ kind: "registered-app-registry", exists: true });
    ...
    expect(registryRunner.deleteValue).toHaveBeenCalledWith(
      registeredApplicationsKey,
      registeredApplicationsValue
    );
    expect(result.removedItems).toHaveLength(1);
    expect(result.removedItems[0]).toMatchObject({
      itemId: path.id,
      path: `${registeredApplicationsKey}\\${registeredApplicationsValue}`,
      registryBackupId: expect.any(String),
      expiresAt: "2026-06-18T00:00:00.000Z"
    });
  ...
  it("backs up and deletes selected app environment variable leftovers after uninstall follow-up", async () => {
    const environmentKey = "HKCU\\Environment";
    const environmentValue = "ACME_NOTES_HOME";
    let valueExists = true;
    const registryRunner = {
    ...
    expect(path).toMatchObject({ kind: "environment-variable-registry", exists: true });
    ...
    expect(registryRunner.deleteValue).toHaveBeenCalledWith(environmentKey, environmentValue);
    expect(result.removedItems).toHaveLength(1);
    expect(result.removedItems[0]).toMatchObject({
      itemId: path.id,
      path: `${environmentKey}\\${environmentValue}`,
      registryBackupId: expect.any(String),
      expiresAt: "2026-06-18T00:00:00.000Z"
    });
  ...
  it("backs up and deletes selected app display cache leftovers after uninstall follow-up", async () => {
    const displayCacheKey =
      "HKCU\\Software\\Classes\\Local Settings\\Software\\Microsoft\\Windows\\Shell\\MuiCache";
    const displayCacheValue = "C:\\Program Files\\Acme Notes\\AcmeNotes.exe.FriendlyAppName";
    let valueExists = true;
    ...
    expect(path).toMatchObject({ kind: "app-display-cache-registry", exists: true });
    ...
    expect(registryRunner.deleteValue).toHaveBeenCalledWith(displayCacheKey, displayCacheValue);
    expect(result.removedItems).toHaveLength(1);
    expect(result.removedItems[0]).toMatchObject({
      itemId: path.id,
      path: `${displayCacheKey}\\${displayCacheValue}`,
      registryBackupId: expect.any(String),
      expiresAt: "2026-06-18T00:00:00.000Z"
    });
  ...
  it("backs up and deletes selected app connection registry leftovers after uninstall follow-up", async () => {
    const openWithRegistryKey = "HKCU\\Software\\Classes\\Applications\\AcmeNotes.exe";
    let keyExists = true;
    const registryRunner = {
      keyExists: vi.fn(async (keyPath: string) => keyExists && keyPath === openWithRegistryKey),
      exportKey: vi.fn(async (_keyPath: string, backupPath: string) => {
        await fs.mkdir(dirname(backupPath), { recursive: true });
        await fs.writeFile(backupPath, registryBackupContentFor(_keyPath), "utf8");
      }),
    ...
    const path = snapshot.groups[0].paths.find((p) => p.path === openWithRegistryKey)!;
    expect(path).toMatchObject({ kind: "open-with-registry", exists: true });
    ...
    expect(registryRunner.deleteKey).toHaveBeenCalledWith(openWithRegistryKey);
    expect(result.removedItems).toHaveLength(1);
    expect(result.removedItems[0]).toMatchObject({
      itemId: path.id,
      path: openWithRegistryKey,
      registryBackupId: expect.any(String),
      expiresAt: "2026-06-18T00:00:00.000Z"
    });
  ...
  it("backs up and deletes selected protocol handler registry leftovers after uninstall follow-up", async () => {
    const protocolRegistryKey = "HKCU\\Software\\Classes\\zoommtg";
    let keyExists = true;
    const registryRunner = {
      valueExists: vi.fn(
      ...
      keyExists: vi.fn(async (keyPath: string) => keyExists && keyPath === protocolRegistryKey),
      exportKey: vi.fn(async (_keyPath: string, backupPath: string) => {
        await fs.mkdir(dirname(backupPath), { recursive: true });
        await fs.writeFile(backupPath, registryBackupContentFor(_keyPath), "utf8");
      }),
    ...
    const snapshot = await planAppLeftovers([], {
      home: fx.home,
      env: { roaming: fx.roaming, localAppData: fx.localAppData, programData: fx.programData },
      extraApps: [{ name: "Zoom", publisher: "Zoom Video Communications" }],
      registryRunner
    });
    const path = snapshot.groups[0].paths.find((p) => p.path === protocolRegistryKey)!;
    expect(path).toMatchObject({ kind: "protocol-handler-registry", exists: true });
    ...
    expect(registryRunner.deleteKey).toHaveBeenCalledWith(protocolRegistryKey);
    expect(result.removedItems).toHaveLength(1);
    expect(result.removedItems[0]).toMatchObject({
      itemId: path.id,
      path: protocolRegistryKey,
      registryBackupId: expect.any(String),
      expiresAt: "2026-06-18T00:00:00.000Z"
    });
  ...
  it("backs up and deletes selected COM local server registry leftovers after uninstall follow-up", async () => {
    const clsidRoot = "HKCU\\Software\\Classes\\CLSID";
    const comRegistryKey =
      `${clsidRoot}\\{A6E0BCA2-2CC0-4B8C-A29D-ABCD00000001}`;
    const unrelatedComRegistryKey =
    ...
    const registryRunner = {
      ...
      queryDefaultValue: vi.fn(async (keyPath: string) => {
        if (keyPath === `${comRegistryKey}\\LocalServer32`) {
          return {
            type: "REG_SZ",
            data: '"C:\\Program Files\\Acme Notes\\AcmeNotes.exe" /automation'
          };
        }
        if (keyPath === `${unrelatedComRegistryKey}\\LocalServer32`) {
          return {
            type: "REG_SZ",
            data: "C:\\Windows\\System32\\notepad.exe"
          };
      ...
      keyExists: vi.fn(async (keyPath: string) => keyExists && keyPath === comRegistryKey),
      exportKey: vi.fn(async (_keyPath: string, backupPath: string) => {
        await fs.mkdir(dirname(backupPath), { recursive: true });
        await fs.writeFile(backupPath, registryBackupContentFor(_keyPath), "utf8");
      }),
    ...
    const paths = snapshot.groups[0].paths.filter((p) => p.kind === "com-local-server-registry");
    ...
    expect(registryRunner.deleteKey).toHaveBeenCalledWith(comRegistryKey);
    expect(result.removedItems).toHaveLength(1);
    expect(result.removedItems[0]).toMatchObject({
      itemId: paths[0].id,
      path: comRegistryKey,
    ...
    expect(registryBackups.entries[0]).toMatchObject({
      appName: "Acme Notes",
      appPublisher: "Acme Corp.",
      keyPath: comRegistryKey,
      backupKind: "com-local-server-key"
    });
  });

  it("backs up and deletes selected COM in-process extension registry leftovers after uninstall follow-up", async () => {
    const clsidRoot = "HKCU\\Software\\Classes\\CLSID";
    const comRegistryKey =
      `${clsidRoot}\\{A6E0BCA2-2CC0-4B8C-A29D-ABCD00000020}`;
    const unrelatedComRegistryKey =
      `${clsidRoot}\\{A6E0BCA2-2CC0-4B8C-A29D-ABCD00000021}`;
    let keyExists = true;
    const registryRunner = {
      listSubKeys: vi.fn(async (keyPath: string) =>
        keyPath === clsidRoot
          ? [
              "{A6E0BCA2-2CC0-4B8C-A29D-ABCD00000020}",
              "{A6E0BCA2-2CC0-4B8C-A29D-ABCD00000021}",
              "AcmeNotes"
            ]
          : []
      ),
      queryDefaultValue: vi.fn(async (keyPath: string) => {
        if (keyPath === `${comRegistryKey}\\InprocServer32`) {
          return {
            type: "REG_SZ",
            data: "C:\\Program Files\\Acme Notes\\AcmeNotesShell.dll"
          };
        }
        if (keyPath === `${unrelatedComRegistryKey}\\InprocServer32`) {
          return {
            type: "REG_SZ",
    ...
    const paths = snapshot.groups[0].paths.filter((p) => p.kind === "com-inproc-server-registry");
    expect(paths).toHaveLength(1);
    expect(paths[0]).toMatchObject({
  ...
  it("backs up and deletes selected right-click menu registry leftovers after uninstall follow-up", async () => {
    ...
    let keyExists = true;
    const registryRunner = {
      keyExists: vi.fn(async (keyPath: string) => keyExists && keyPath === contextMenuRegistryKey),
      exportKey: vi.fn(async (_keyPath: string, backupPath: string) => {
        await fs.mkdir(dirname(backupPath), { recursive: true });
        await fs.writeFile(backupPath, registryBackupContentFor(_keyPath), "utf8");
      }),
    ...
    const path = snapshot.groups[0].paths.find((p) => p.path === contextMenuRegistryKey)!;
    expect(path).toMatchObject({ kind: "context-menu-registry", exists: true });
    ...
    expect(registryRunner.exportKey).toHaveBeenCalledWith(
      contextMenuRegistryKey,
      expect.stringMatching(/backup\.reg$/)
    );
    expect(registryRunner.deleteKey).toHaveBeenCalledWith(contextMenuRegistryKey);
    expect(result.removedItems).toHaveLength(1);
    expect(result.removedItems[0]).toMatchObject({
      itemId: path.id,
      path: contextMenuRegistryKey,
      registryBackupId: expect.any(String),
      expiresAt: "2026-06-18T00:00:00.000Z"
    });
  ...
  it("backs up and deletes a selected Windows service trace after uninstall follow-up", async () => {
    const serviceKeyPath = "HKLM\\SYSTEM\\CurrentControlSet\\Services\\AcmeNotesSvc";
    ...
    const startupEntries: StartupAutoEntry[] = [
      {
        ...
        path: "C:\\Program Files\\Acme Notes\\helper.exe",
        publisher: "Acme Corp.",
        origin: "Windows 서비스",
        enabled: true
      }
    ...
    const path = snapshot.groups[0].paths.find((p) => p.startupEntryKind === "service")!;
    expect(path).toMatchObject({
      kind: "startup-entry",
      serviceName: "AcmeNotesSvc",
      protectedBy: undefined
    ...
    expect(result.removedItems[0]).toMatchObject({
      itemId: path.id,
      path: "Windows 서비스: Acme Notes Helper",
      registryBackupId: expect.any(String),
      expiresAt: "2026-06-18T00:00:00.000Z"
    });
