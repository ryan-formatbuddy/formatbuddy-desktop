...
describe("registry leftover cleanup", () => {
  ...
  it("only allows narrow COM in-process extension registry keys", () => {
    expect(
      isSafeComInprocServerRegistryKeyPath(
        "HKCU\\Software\\Classes\\CLSID\\{A6E0BCA2-2CC0-4B8C-A29D-ABCD00000020}"
      )
    ).toBe(true);
    expect(
      isSafeComInprocServerRegistryKeyPath(
        "HKLM\\Software\\WOW6432Node\\Classes\\CLSID\\{A6E0BCA2-2CC0-4B8C-A29D-ABCD00000020}"
      )
    ).toBe(true);
    expect(
      isSafeComInprocServerRegistryKeyPath(
        "HKCU\\Software\\Classes\\CLSID\\{A6E0BCA2-2CC0-4B8C-A29D-ABCD00000020}\\InprocServer32"
      )
    ).toBe(false);
    expect(
      isSafeComInprocServerRegistryKeyPath("HKCU\\Software\\Classes\\CLSID\\AcmeNotes")
    ).toBe(false);
    expect(
      isSafeComInprocServerRegistryKeyPath(
        "HKCU\\Software\\Classes\\Interface\\{A6E0BCA2-2CC0-4B8C-A29D-ABCD00000020}"
      )
    ).toBe(false);
  });

  it("only allows narrow COM AppID registry keys", () => {
    expect(
      isSafeComAppIdRegistryKeyPath(
        "HKCU\\Software\\Classes\\AppID\\{A6E0BCA2-2CC0-4B8C-A29D-ABCD00000010}"
      )
    ).toBe(true);
    expect(
      isSafeComAppIdRegistryKeyPath(
        "HKLM\\Software\\Classes\\AppID\\{A6E0BCA2-2CC0-4B8C-A29D-ABCD00000010}"
      )
    ).toBe(true);
    expect(
      isSafeComAppIdRegistryKeyPath(
        "HKLM\\Software\\WOW6432Node\\Classes\\AppID\\{A6E0BCA2-2CC0-4B8C-A29D-ABCD00000010}"
      )
    ).toBe(true);
    expect(
      isSafeComAppIdRegistryKeyPath(
        "HKCU\\Software\\Classes\\AppID\\{A6E0BCA2-2CC0-4B8C-A29D-ABCD00000010}\\Extra"
      )
    ).toBe(false);
    expect(isSafeComAppIdRegistryKeyPath("HKCU\\Software\\Classes\\AppID\\AcmeNotes")).toBe(false);
    expect(
      isSafeComAppIdRegistryKeyPath(
        "HKCU\\Software\\Classes\\CLSID\\{A6E0BCA2-2CC0-4B8C-A29D-ABCD00000010}"
      )
    ).toBe(false);
  });
  ...
  it("exports a backup before deleting an app COM in-process extension registry key", async () => {
    const keyPath =
      "HKCU\\Software\\Classes\\CLSID\\{A6E0BCA2-2CC0-4B8C-A29D-ABCD00000020}";
    let keyExists = true;
    const calls: string[] = [];
    ...
    expect(restored).toMatchObject({
      status: "restored",
      keyPath,
      message: "앱 확장 연결 백업을 되돌렸어요."
    });
    expect(restored.entry).toMatchObject({ backupKind: "com-inproc-server-key" });
    expect(onAppRegistryBackupRestored).toHaveBeenCalledWith({
      name: "Acme Notes",
      publisher: "Acme Corp.",
      backupKind: "com-inproc-server-key",
  ...
  it("exports a backup before deleting an app COM AppID registry key", async () => {
    const keyPath =
      "HKCU\\Software\\Classes\\AppID\\{A6E0BCA2-2CC0-4B8C-A29D-ABCD00000010}";
    let keyExists = true;
    const calls: string[] = [];
    ...
    expect(restored).toMatchObject({
      status: "restored",
      keyPath,
      message: "앱 실행 연결 정보 백업을 되돌렸어요."
    });
    expect(restored.entry).toMatchObject({ backupKind: "com-app-id-key" });
    expect(onAppRegistryBackupRestored).toHaveBeenCalledWith({
      name: "Acme Notes",
      publisher: "Acme Corp.",
      backupKind: "com-app-id-key",
