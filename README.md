...
- 오래된 Windows 오류 리포트 / 앱 크래시 덤프를 검토형 정리 후보로 표시
- Windows 휴지통은 자동 비움 대신 직접 확인 안내 (포맷버디 30일 복구함 원칙 유지)
- 한국 특화 앱 14종 잔여 후보 인식 (V3 / 한컴 / 더존 / 카카오게임즈 / 토스 등)
- 앱 제거 후 남은 기본 앱 목록·PATH 경로·실행 경로·앱 연결·프로토콜 연결·브라우저 연결 도우미·우클릭 메뉴·서비스 흔적을 백업 후 정리
...
- scheduled task cleanup traces can be backed up, removed, and restored
- app deletion registry traces such as uninstall keys, default-app list values, App Paths, Open With, URL protocol handlers, browser connection helpers, right-click menu keys, user PATH segments, and services can be backed up, removed, and restored
...
tasks, app-specific registry keys, and temporary files, then removes them.
